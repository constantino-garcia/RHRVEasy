...
RHRVEasyPValues <-
  function(folders,
    ...
    easy_options <- list("significance" = significance_level,
                         "verbose" = verbose)
    ...
    for (folder in folders) {
      ...
      dataFrameMTime = rbind(dataFrameMTime,
                             dataFrameMTime = time_analysis(format,
                                                            f[[i]], split_path(folder)[1], folder, ...))
      if (nonLinear == TRUE) {
        cat("Performing non linear analysis...")
        dataFrameMNonLinear = rbind(
          dataFrameMNonLinear,
          non_linear_analysis(format,
                              f[[i]], split_path(folder)[1], folder, ...)
        )
      }
      i = i + 1
    ...
    if (typeAnalysis == "fourier") {
      i = 1
      for (folder in folders) {
        dataFrameMFreq = rbind(dataFrameMFreq,
                               dataFrameMFreq = freq_analysis(format,
                                                              f[[i]], split_path(folder)[1], folder, ...))
        i = i + 1
      }
    ...
    if (typeAnalysis == "wavelet") {
      ...
      for (folder in folders) {
        dataFrameMWavelet = rbind(
          dataFrameMWavelet,
          dataFrameMWavelet = wavelet_analysis(format,
                                               f[[i]], split_path(folder)[1], folder,
                                               type = typeAnalysis, ...)
        )
        i = i + 1
  ...
  for (correct in corrections) {
    for (i in 1:100) {
      a = RHRVEasyPValues(
        folders = records,
        correction = TRUE,
        correctionMethod = correct
      )
      for (column in columNames) {
        if (a$pValues[[column]] < 0.05) {
          significativityCount[[correct]][[column]] = significativityCount[[correct]][[column]] +
            1
...
res = cbind(
  significativityCount$none,
  significativityCount$fdr,
  significativityCount$holm,
  significativityCount$hochberg,
  significativityCount$BY,
  significativityCount$bonferroni
)
