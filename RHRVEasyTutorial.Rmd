...
```{r}
base_path = "rrs/" 
NSR_DB = file.path(base_path, "normal")
CHF_DB = file.path(base_path, "chf")
NSR_HALF_DB = file.path(base_path, "normal_half")
CHF_HALF_DB = file.path(base_path, "chf_half")
```
...
The significance level can be changed with the parameter `significance` (the default value is 0.05):

```{r}
RHRVEasy_obj_001 = RHRVEasy(folders = c(NSR_DB, CHF_DB), significance = 0.01)
...
If the argument `saveHRVindicesInPath` is specified when invoking the function `RHRVEasy`, an Excel spreadsheet with all the HRV indices calculated for each recording will be created in the path specified specified in this parameter. The name of the spreadsheet generated is "<group 1 name> vs <group 2 name> .xlsx":

```{r}
RHRVEasy_obj = RHRVEasy(folders =c(NSR_DB, CHF_DB), saveHRVindicesInPath = spreadsheet_path)
```

This spreadsheet can also be generated from the object returned by `RHRVEasy` by calling the function `saveHRVindices`.

```{r}
saveHRVindices (RHRVEasy_obj, saveHRVindicesInPath = spreadsheet_path)
```
...
saveHRVindices (RHRVEasy_obj_four_groups, saveHRVindicesInPath = spreadsheet_path)
```
...
**Warning**: the following sentence, commented in the tutorial, will take several hours to execute on a medium to high performance PC, and possibly several days on a low performance PC.
...
# #Let us save the indices
# saveHRVindices (RHRVEasy_obj_nonLinear, saveHRVindicesInPath = spreadsheet_path)
```
